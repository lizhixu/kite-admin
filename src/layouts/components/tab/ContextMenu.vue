...
const options = computed(() => [
  {
    ...
    icon: () => h('i', { class: 'i-fe:refresh-cw text-14' }),
  },
  {
    label: '关闭',
    key: 'close',
    disabled: tabStore.tabs.length <= 1,
    icon: () => h('i', { class: 'i-fe:x text-14' }),
  },
  {
    label: '关闭其他',
    key: 'close-other',
    disabled: tabStore.tabs.length <= 1,
    icon: () => h('i', { class: 'i-fe:maximize-2 text-14' }),
  },
  {
    label: '关闭左侧',
    key: 'close-left',
    disabled: tabStore.tabs.length <= 1 || props.currentPath === tabStore.tabs[0].path,
    icon: () => h('i', { class: 'i-fe:arrow-left text-14' }),
  },
  {
    label: '关闭右侧',
    key: 'close-right',
    ...
    icon: () => h('i', { class: 'i-fe:arrow-right text-14' }),
  },
])
